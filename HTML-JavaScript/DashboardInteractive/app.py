...
def metadata(sample):
    ...
    for info in samples:
        if (sample_id == info.sample):
            ...
            sample_metadata["SAMPLEID"] = info.sample
...
def samples(sample):
    ...
    sample_id = str(sample.split("_")[1])
    # Create sample query
    sample_query = "Samples.'" + sample_id + "'"
    ...
    # Grab info
    results = session.query(Samples.otu_id, sample_query,Samples.otu_label).order_by(desc(sample_query))
    # Loop through & append
    for result in results:
        otu_ids.append(result[0])
        sample_values.append(result[1])
        sample_labels.append(result[2])
